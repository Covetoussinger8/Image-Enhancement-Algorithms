...
def maxEntropyEnhance(I, isBad, a=-0.3293, b=1.1258):
    ...
    isBad = isBad * 1
    isBad = np.array(
        Image.fromarray((isBad * 255).astype(np.uint8)).resize((50, 50), 3))
...
def u(s, a):
    if (abs(s) >= 0) & (abs(s) <= 1):
        ...
        return a * (abs(s) ** 3) - (5 * a) * (abs(s) ** 2) + (8 * a) * abs(
            s) - 4 * a
    return 0
...
def Ying_2017_CAIP(img, mu=0.5, a=-0.3293, b=1.1258):
    ...
    t_b = np.max(I, axis=2)
    t_our = cv2.resize(tsmooth(np.array(
        Image.fromarray((t_b * 255).astype(np.uint8)).resize((50, 50), 3)),
                               lamda, sigma), (t_b.shape[1], t_b.shape[0]),
